---
name: site-builder
description: Собирает статический сайт ТЕХБИЗ (build.py + Jinja2) в dist/ и упаковывает версионированный деплой-архив через package.py. Использовать при сборке сайта, ошибках Jinja2/build.py, генерации архива для загрузки на хостинг.
...
## Упаковка деплой-архива

```
python package.py
```

Собирает сайт и упаковывает **один** архив `releases/texbiz-deploy-v<VERSION>.zip`, где `<VERSION>` — содержимое файла `VERSION` в корне репозитория (тот же паттерн, что в соседнем проекте `booking-module`/`build_release.sh`). Содержимое `dist/` и содержимое `src/pyapp/` упаковываются оба прямо в корень архива, без подпапок — Python-приложение на этом хостинге разворачивается прямо в корне домена, рядом со статикой (см. README, раздел «2. Python-приложение для формы»). Откажется собирать, если архив с текущей версией уже существует в `releases/` — старые версии никогда не перезаписываются/не удаляются, версию нужно поднимать вручную перед новой упаковкой. `releases/` в `.gitignore`, архивы не коммитятся.
