...
### Что делает .htaccess и почему без него нельзя

Корень домена совпадает с корнем проекта, поэтому Apache видит все файлы
приложения и отдаёт файлом всё, что физически существует. Без правил из
`.htaccess` по прямой ссылке открываются `data/db.sqlite3` со всеми заявками
и `.env` с паролем от почты. Файл закрывает каталоги `data/`, `config/`,
`web/`, `blog/`, `scripts/`, `templates/`, файлы с расширениями кода и базы,
а также всё, что начинается с точки.

Он же перебрасывает `/static/` и `/media/` на `public/static/` и
`public/media/`: в боевом режиме Django статику не отдаёт, и без переброски
сайт открывается без стилей. Если в панели удастся задать Alias на уровне
сервера, это надёжнее — тогда два правила переброски можно убрать.

После выкладки обязательно проверить вручную: `https://tex-biz.ru/data/db.sqlite3`
и `https://tex-biz.ru/.env` должны отдавать 403 или 404, но не файл.
